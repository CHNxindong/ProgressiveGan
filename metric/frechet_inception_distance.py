...
#example usage :python metrics/frechet_inception_distance.py "/home/guyuchao/ssd/PycharmProjects/testgan/real" "/home/guyuchao/ssd/PycharmProjects/testgan/generate"  --gpu="1"
...
def calculate_activation_statistics(files, sess, batch_size=64, verbose=False):
    ...
    """
    total=len(files)
    iter_num=total//batch_size
    act=np.zeros((total,2048))
    for i in tqdm(range(iter_num)):
        x = np.array([imread(str(fn)).astype(np.float32) for fn in files[i*batch_size:(i+1)*batch_size]])
        act[i*batch_size:(i+1)*batch_size]=get_activations(x,sess,batch_size)
...
def _handle_path(path, sess):
    if path.endswith('.npz'):
        ...
        m, s = calculate_activation_statistics(files, sess)
    return m, s
