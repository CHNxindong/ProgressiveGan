...
class API:
    def __init__(self, num_images,minibatch_size):
...
def get_images_np(files):
    return np.concatenate(np.array([np.expand_dims(imread(str(fn)).astype(np.float32), 0) for fn in files]),0)
#----------------------------------------------------------------------------
if __name__=="__main__":
    import glob
    from tqdm import tqdm
    api=API(num_images=5000,minibatch_size=100)
    path = glob.glob("/home/guyuchao/ssd/PycharmProjects/testgan/generate2/*.jpg")
    for i in tqdm(range(100)):
        api.feed(get_images_np(path[i*50:(i+1)*50]))
